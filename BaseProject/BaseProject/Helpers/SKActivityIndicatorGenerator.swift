//
//  SKActivityIndicatorGenerator.swift
//  WalkAndTalk
//
...
class SKActivityIndicatorStyle: NSObject {
     ...
     */
    class func createSpinner(in layer: CALayer, size: CGSize, colors: [UIColor], style: ActivityIndicatorStyle) {
        
        switch  style {
        case .defaultSpinner:
            self.shared.createCircularLineFadingActivityIndicator(in: layer, size: size, colors: colors)
            
        case .spinningFadeCircle:
            self.shared.createCircularSpinningBallWithFadingActivityIndicator(in: layer, size: size, colors: colors)
            
        case .spinningCircle:
            self.shared.createCircularSpinninBallActivityIndicator(in: layer, size: size, colors: colors)
            
        case .spinningHalfCircles:
            self.shared.createTwoHalfCircleSpinningActivityIndicator(in: layer, size: size, color: colors[0])
        }
    }
    ...
    // ---------------------------------------------------Line Fade Spinner ---------------------------------------------- //
    // ------------------------------------------------------------------------------------------------------------------- //
    func createCircularLineFadingActivityIndicator(in layer: CALayer, size: CGSize, colors: [UIColor]) {
        ...
        while newColors.count < 10 {
            newColors.append(contentsOf: colors.reversed())
        }
    ...
    func createCircularSpinningBallWithFadingActivityIndicator(in layer: CALayer, size: CGSize, colors: [UIColor]) {
        ...
        scaleAnimation.values = [1, 0.6, 0.2]
        ...
        opacityAnimaton.values = [1, 0.6, 0.3]
    ...
    // ---------------------------------------------------Ball Spin Spinner ---------------------------------------------- //
    // ------------------------------------------------------------------------------------------------------------------- //
    func createCircularSpinninBallActivityIndicator(in layer: CALayer, size: CGSize, colors: [UIColor]) {
        let circleSize = size.width / 8
        ...
        // Draw circles
        for i in 0 ..< 8 {
            let factor = Float(i) * 1 / 8
            let circle = circularSpinningBallActivityIndicatorlayerWith(size: CGSize(width: circleSize, height: circleSize), color: newColors[i])
    ...
    func rotateAnimation(_ rate: Float, x: CGFloat, y: CGFloat, size: CGSize) -> CAAnimationGroup {
        let duration: CFTimeInterval = 2.7
